...
async def download_file(
    plugin, event, file_item: dict, user_config: dict, full_path_override: str = None
):
    ...
    if not plugin._is_extension_allowed(file_name, user_config):
        ...
        return
    max_download_size_mb = plugin._get_size_limit_mb(
        user_config, "max_download_size", 50
    )
    ...
    try:
        ...
        async with plugin._create_openlist_client(user_config) as client:
            ...
            if not link:
                yield event.plain_result(
                    "❌ 无法获取真实下载链接，请确认配置账号为 OpenList 管理员或具有 /api/fs/link 权限"
                )
                return
            download_url = link["url"]
            download_headers = plugin._normalize_download_headers(
                link.get("header", {})
            )
            ...
            if max_download_size_mb > 0 and link_size > max_download_size:
                ...
                return
            downloads_dir = Path(
                build_plugin_temp_dir(get_astrbot_temp_path(), "downloads")
            )
            ensure_dir(downloads_dir)
            safe_filename = plugin._sanitize_filename(file_name)
            temp_file_path = build_temp_file_path(
                str(downloads_dir), user_id, plugin._unique_suffix(), safe_filename
            )
            yield event.plain_result(
        ...
        logger.error(
            f"用户 {user_id} 下载文件失败: {e}, 文件: {file_name}, 路径: {file_path}",
            exc_info=True,
        )
        yield event.plain_result(
            f"❌ 下载失败: {str(e)}\n💡 提示: 管理员可在后台日志中查看详细错误信息"
...
async def get_and_send_download_link(
    plugin, event, item: dict, user_config: dict, full_path: str = None
):
    user_id = event.get_sender_id()
    ...
    try:
        async with plugin._create_openlist_client(user_config) as client:
            ...
            if download_url:
                ...
                logger.warning(
                    f"用户 {user_id} 无法获取下载链接 - 路径: {file_path}, 文件名: {item.get('name', '')}"
                )
                yield event.plain_result(
                    f"❌ 无法获取下载链接，文件可能不存在或为目录: {file_path}"
                )
    except Exception as e:
        logger.error(
            f"用户 {user_id} 获取下载链接失败: {e}, 路径: {file_path}, 文件名: {item.get('name', '')}",
            exc_info=True,
        )
        yield event.plain_result(
            f"❌ 操作失败: {str(e)}\n💡 提示: 管理员可在后台日志中查看详细错误信息"
...
async def handle_file_info(plugin, event, path: str):
    ...
    try:
        ...
        logger.error(
            f"用户 {user_id} 获取文件信息失败: {e}, 路径候选: {path_candidates}",
            exc_info=True,
        )
        yield event.plain_result(
            f"❌ 操作失败: {str(e)}\n💡 提示: 管理员可在后台日志中查看详细错误信息"
...
async def handle_download_command(plugin, event, path: str):
    ...
    if item_to_download:
        async for result in download_file(
            plugin,
            event,
            item_to_download,
            user_config,
            full_path_override=full_path_override,
        ):
            yield result
            ...
            if use_group_upload:
                try:
                    root_files = await event.bot.api.call_action(
                        "get_group_root_files", group_id=target_group_id
                    )
            ...
            for i, item in enumerate(files_to_restore, 1):
                ...
                try:
                    ...
                    if (
                        ...
                        and item_size
                        and item_size > max_download_size
                    ):
                        logger.debug(
                            f"跳过恢复文件 {file_name}: 大小 {item_size} 超过限制 {max_download_size_mb}MB。"
                        )
                        fail_count += 1
                        continue
                    ...
                    if max_download_size_mb > 0 and link_size > max_download_size:
                        logger.debug(
                            f"跳过恢复文件 {file_name}: 下载链接大小 {link_size} 超过限制 {max_download_size_mb}MB。"
                        )
                        fail_count += 1
                        continue
                    ...
                    safe_filename = plugin._sanitize_filename(file_name)
                    temp_file_path = build_temp_file_path(
                        downloads_dir, "restore", plugin._unique_suffix(), safe_filename
                    )
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            download_url, headers=download_headers
                            ...
                            if response.status == 200:
                                ...
                                logger.error(
                                    f"下载失败 {file_name}: HTTP {response.status}"
                                )
                                fail_count += 1
                                continue
                    ...
                    if use_group_upload:
                        ...
                        try:
                            ...
                            await event.bot.api.call_action(
                                "upload_group_file", **upload_params
                            )
                            success_count += 1
                        except Exception as e:
                    ...
                    if i % 5 == 0 or i == total:
                        logger.info(
                            f"📧 恢复进度: {i}/{total} (成功: {success_count}, 失败: {fail_count})"
                        )
                except Exception as e:
...
async def handle_preview_command(plugin, event, path: str):
    ...
    try:
        async with plugin._create_openlist_client(user_config) as client:
            ...
            if (
                max_preview_size_mb > 0
                and file_size > max_preview_size_mb * 1024 * 1024
            ):
                yield event.plain_result(
            ...
            if not link:
                yield event.plain_result(
                    "❌ 获取真实下载链接失败，请确认配置账号为 OpenList 管理员或具有 /api/fs/link 权限"
                )
                return
            download_url = link["url"]
            download_headers = plugin._normalize_download_headers(
            ...
            safe_filename = plugin._sanitize_filename(file_name)
            temp_file_path = (
                temp_dir / f"preview_{plugin._unique_suffix()}_{safe_filename}"
            )
            ...
            try:
                ...
                if ext == ".epub":
                    ...
                    preview_text = extract_epub_preview_text(
                        str(temp_file_path), text_length
                    )
                    ...
                    try:
                        with open(temp_file_path, "rb") as f:
                            ...
                            logger.debug(
                                f"文本预览编码检测: {preview['encoding']}, 置信度: {preview['confidence']:.2f}"
                            )
                            yield event.plain_result(
                                f"📝 文本预览:\n---\n{preview['text']}"
                            )
