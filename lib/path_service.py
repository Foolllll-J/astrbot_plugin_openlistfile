...
def resolve_target_path(
    current_path: str, path: str, default_to_current: bool = True
) -> str:
    raw_path = (path or "").strip()
    normalized_current = (
        current_path
        if isinstance(current_path, str) and current_path.startswith("/")
        else "/"
    )
    if not raw_path:
...
def resolve_path_candidates(
    ...
    raw_path = (path or "").strip()
    primary_path = resolve_target_path(
        current_path, raw_path, default_to_current=default_to_current
    )
...
def strip_fixed_base_directory(path: str, fixed_base_directory: str) -> str:
    ...
    if fixed_base_dir != "/" and (
        normalized_path == fixed_base_dir
        or normalized_path.startswith(fixed_base_dir + "/")
    ):
...
def get_item_full_path(
    item: Dict, current_path: str, fixed_base_directory: str = ""
) -> str:
